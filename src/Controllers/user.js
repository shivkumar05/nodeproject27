...
const getMyDrills = async function (req, res) {
    try {
        ...
        const drills = await myDrillModel.find({ userId: userid, isCompleted: false, $or: [filter] }).select({ createdAt: 0, updatedAt: 0, __v: 0 });
...
const getPastDrill = async function (req, res) {
    try {
        ...
        const drills = await myDrillModel.find({ userId: userid, isCompleted: true, $or: [filter] }).select({ createdAt: 0, updatedAt: 0, __v: 0 });
...
let getAllUsers = async function (req, res) {
    try {
        ...
        var obj = {};
        var allUser = await userModel.find({ academy_id: userid });
        for (var i=0; i<allUser.length; i++) {
            obj[i] = allUser[i];
            // console.log(allUser[i], "bbbb")
        
        var UserProfile = await profileModel.findOne({ userId: obj[i]._id }).select({ _id: 0, createdAt: 0, updatedAt: 0, __v: 0 });
        obj.userProfile = UserProfile;
            
        }
        // console.log(UserProfile, "qqqqq")
